...
/* =====================================================================
   34. AES explorer: a complete, verified AES-128 in plain JS (it passes the
   FIPS-197 test vector). Tab 1 animates the 16-byte state through 10 rounds;
   tab 2 is the full 16x16 S-box; tab 3 contrasts ECB, CBC and CTR on four
   identical plaintext blocks. No libraries. (AES page)
   ===================================================================== */
// prettier-ignore
...
type AesState = number[][]; // [row][col], column-major load

function aesKeyExpansion(key: Uint8Array): number[][] {
  ...
  for (let i = 4; i < 44; i++) {
    let t = w[i - 1].slice();
    if (i % 4 === 0) {
      t = [t[1], t[2], t[3], t[0]].map((b) => AES_SBOX[b]);
      t[0] ^= Rcon[i / 4 - 1];
    }
...
const aesStateFrom = (bytes: Uint8Array): AesState => {
  const s: AesState = [[], [], [], []];
  for (let i = 0; i < 16; i++) s[i % 4][Math.floor(i / 4)] = bytes[i];
  return s;
};
const aesStateBytes = (s: AesState): Uint8Array => {
  const o = new Uint8Array(16);
  for (let i = 0; i < 16; i++) o[i] = s[i % 4][Math.floor(i / 4)];
  return o;
};
const aesAddRoundKey = (s: AesState, w: number[][], rnd: number) => {
  for (let c = 0; c < 4; c++) for (let r = 0; r < 4; r++) s[r][c] ^= w[rnd * 4 + c][r];
};
const aesSubBytes = (s: AesState) => {
  for (let r = 0; r < 4; r++) for (let c = 0; c < 4; c++) s[r][c] = AES_SBOX[s[r][c]];
};
const aesShiftRows = (s: AesState) => {
  for (let r = 1; r < 4; r++) s[r] = s[r].slice(r).concat(s[r].slice(0, r));
};
const aesMixColumns = (s: AesState) => {
  for (let c = 0; c < 4; c++) {
    const a = [s[0][c], s[1][c], s[2][c], s[3][c]];
    s[0][c] = aesGmul(a[0], 2) ^ aesGmul(a[1], 3) ^ a[2] ^ a[3];
    s[1][c] = a[0] ^ aesGmul(a[1], 2) ^ aesGmul(a[2], 3) ^ a[3];
    s[2][c] = a[0] ^ a[1] ^ aesGmul(a[2], 2) ^ aesGmul(a[3], 3);
    s[3][c] = aesGmul(a[0], 3) ^ a[1] ^ a[2] ^ aesGmul(a[3], 2);
  }
};
const aesInvShiftRows = (s: AesState) => {
  for (let r = 1; r < 4; r++) s[r] = s[r].slice(4 - r).concat(s[r].slice(0, 4 - r));
};
const aesInvSubBytes = (s: AesState) => {
  for (let r = 0; r < 4; r++) for (let c = 0; c < 4; c++) s[r][c] = AES_INV_SBOX[s[r][c]];
};
const aesInvMixColumns = (s: AesState) => {
  for (let c = 0; c < 4; c++) {
    const a = [s[0][c], s[1][c], s[2][c], s[3][c]];
    s[0][c] = aesGmul(a[0], 14) ^ aesGmul(a[1], 11) ^ aesGmul(a[2], 13) ^ aesGmul(a[3], 9);
    s[1][c] = aesGmul(a[0], 9) ^ aesGmul(a[1], 14) ^ aesGmul(a[2], 11) ^ aesGmul(a[3], 13);
    s[2][c] = aesGmul(a[0], 13) ^ aesGmul(a[1], 9) ^ aesGmul(a[2], 14) ^ aesGmul(a[3], 11);
    s[3][c] = aesGmul(a[0], 11) ^ aesGmul(a[1], 13) ^ aesGmul(a[2], 9) ^ aesGmul(a[3], 14);
  }
};
...
function aesEncryptCapture(pt: Uint8Array, key: Uint8Array): { ct: Uint8Array; snaps: AesSnap[] } {
  const w = aesKeyExpansion(key);
  const s = aesStateFrom(pt);
  const snaps: AesSnap[] = [];
  aesAddRoundKey(s, w, 0);
  snaps.push({ label: "round 0 . AddRoundKey", bytes: aesStateBytes(s) });
  for (let rnd = 1; rnd < 10; rnd++) {
    aesSubBytes(s);
    aesShiftRows(s);
    aesMixColumns(s);
    aesAddRoundKey(s, w, rnd);
    snaps.push({ label: `round ${rnd} . SubBytes ShiftRows MixColumns AddRoundKey`, bytes: aesStateBytes(s) });
  }
  aesSubBytes(s);
  aesShiftRows(s);
  aesAddRoundKey(s, w, 10);
  snaps.push({ label: "round 10 . final (no MixColumns)", bytes: aesStateBytes(s) });
  return { ct: aesStateBytes(s), snaps };
}
function aesDecryptBlock(ct: Uint8Array, key: Uint8Array): Uint8Array {
  const w = aesKeyExpansion(key);
  const s = aesStateFrom(ct);
  aesAddRoundKey(s, w, 10);
  ...
  aesInvSubBytes(s);
  aesAddRoundKey(s, w, 0);
  return aesStateBytes(s);
}
const aesEcb = (blocks: Uint8Array[], key: Uint8Array) => blocks.map((b) => aesEncryptCapture(b, key).ct);
const aesCbc = (blocks: Uint8Array[], key: Uint8Array, iv: Uint8Array) => {
  const out: Uint8Array[] = [];
  let prev = iv;
  for (const b of blocks) {
    const x = Uint8Array.from(b.map((v, i) => v ^ prev[i]));
    const c = aesEncryptCapture(x, key).ct;
    out.push(c);
    prev = c;
  }
  return out;
};
const aesCtr = (blocks: Uint8Array[], key: Uint8Array, nonce: Uint8Array) =>
  blocks.map((b, i) => {
    const ctr = nonce.slice();
    ctr[15] = i;
    const ks = aesEncryptCapture(ctr, key).ct;
    return Uint8Array.from(b.map((v, j) => v ^ ks[j]));
  });

const aesByteColor = (b: number) => `hsl(${(b * 1.41) % 360}, 65%, 55%)`;
const aesHex = (bytes: Uint8Array) => Array.from(bytes).map((b) => b.toString(16).padStart(2, "0")).join("");

type AesTab = "cipher" | "sbox" | "modes";

function AesExplorerWidget() {
  const [tab, setTab] = useState<AesTab>("cipher");

  /* ---- tab 1: encrypt / decrypt ---- */
  const [msg, setMsg] = useState("Send 1 BTC to Alice");
  const [keyHex, setKeyHex] = useState("2b7e151628aed2a6abf7158809cf4f3c");
  const [snaps, setSnaps] = useState<AesSnap[]>([]);
  const [snapIdx, setSnapIdx] = useState(0);
  const [ct, setCt] = useState<Uint8Array | null>(null);
  const [recovered, setRecovered] = useState<string | null>(null);
  const aesTimer = useRef<number | null>(null);

  const keyBytes = useMemo(() => {
    const clean = keyHex.replace(/[^0-9a-fA-F]/g, "").slice(0, 32).padEnd(32, "0");
    return shaHexToBytes(clean);
  }, [keyHex]);
  const ptBytes = useMemo(() => {
    const enc = new TextEncoder().encode(msg).slice(0, 16);
  ...
  useEffect(
    () => () => {
      if (aesTimer.current) clearInterval(aesTimer.current);
    },
    [],
  );
  ...
  const runEncrypt = () => {
    if (aesTimer.current) clearInterval(aesTimer.current);
    const { ct: out, snaps: sn } = aesEncryptCapture(ptBytes, keyBytes);
    setSnaps(sn);
    setCt(out);
    setRecovered(null);
    setSnapIdx(0);
    let i = 0;
    aesTimer.current = window.setInterval(() => {
      i += 1;
      if (i >= sn.length) {
        if (aesTimer.current) clearInterval(aesTimer.current);
        return;
  ...
  const runDecrypt = () => {
    if (!ct) return;
    const pt = aesDecryptBlock(ct, keyBytes);
    const text = new TextDecoder().decode(pt).replace(/ +$/, "");
    setRecovered(text);
  };

  const curState = snaps.length ? snaps[snapIdx].bytes : ptBytes;

  /* ---- tab 2: s-box ---- */
  const [sboxIn, setSboxIn] = useState("53");
  const sboxByte = parseInt(sboxIn || "0", 16) & 0xff;

  /* ---- tab 3: modes ---- */
  const modes = useMemo(() => {
    const block = new Uint8Array(16);
    new TextEncoder().encode("ATTACK AT DAWN!!").forEach((v, i) => (block[i] = v));
    const blocks = [block, block, block, block];
    const key = shaHexToBytes("2b7e151628aed2a6abf7158809cf4f3c");
    const iv = shaHexToBytes("000102030405060708090a0b0c0d0e0f");
    const nonce = shaHexToBytes("aa000000000000000000000000000000");
    return {
      plain: blocks,
      ecb: aesEcb(blocks, key),
      cbc: aesCbc(blocks, key, iv),
      ctr: aesCtr(blocks, key, nonce),
    };
  }, []);

  const blockRow = (b: Uint8Array, key: string) => (
    <div className="aes-mblock" key={key}>
      {Array.from(b).map((v, i) => (
        <span key={i} className="aes-mcell" style={{ background: aesByteColor(v) }} />
      ))}
    </div>
  );
  ...
  return (
    <div className="widget-wrap">
      <div className="widget-head">
        <span className="widget-title">{"// watch AES transform a block"}</span>
        <div className="widget-controls">
          <button type="button" className={`widget-btn ${tab === "cipher" ? "is-active" : ""}`} onClick={() => setTab("cipher")}>
            encrypt
          </button>
          <button type="button" className={`widget-btn ${tab === "sbox" ? "is-active" : ""}`} onClick={() => setTab("sbox")}>
            s-box
          </button>
          <button type="button" className={`widget-btn ${tab === "modes" ? "is-active" : ""}`} onClick={() => setTab("modes")}>
            modes
          </button>
        </div>
      </div>
      ...
      {/* TAB 1 */}
      {tab === "cipher" && (
        <div className="aes-panel">
          <div className="cx-fields">
            <label className="cx-field">
              <span>message (first 16 bytes)</span>
              <input value={msg} onChange={(e) => setMsg(e.target.value)} aria-label="message" />
            </label>
            <label className="cx-field">
              <span>key (32 hex = AES-128)</span>
              <input className="sig-mono" value={keyHex} onChange={(e) => setKeyHex(e.target.value)} aria-label="key hex" />
            </label>
          </div>
          <div className="aes-cipher-controls">
            <button type="button" className="widget-btn gx-go" onClick={runEncrypt}>
              encrypt . animate 10 rounds
            </button>
            <button type="button" className="widget-btn" onClick={runDecrypt} disabled={!ct}>
              decrypt
            </button>
            {snaps.length > 0 && <span className="aes-round-label">{snaps[snapIdx].label}</span>}
          </div>

          <div className="aes-state">
            {Array.from({ length: 16 }).map((_, i) => {
              // display column-major: cell (r,c) shows byte index r + 4c
              const r = i % 4;
              const c = Math.floor(i / 4);
              const v = curState[r + 4 * c];
              return (
                <span key={i} className="aes-state-cell" style={{ background: aesByteColor(v) }}>
          ...
          {ct && (
            <div className="aes-out">
              <div className="sig-field">
                <span>ciphertext (16 bytes)</span>
                <code className="sig">{aesHex(ct)}</code>
              </div>
              {recovered !== null && (
                <div className="aes-verdict ok">decrypt recovered: &quot;{recovered}&quot;</div>
              )}
            </div>
          )}
          <p className="widget-caption">
            this is a real AES-128 running in your browser, and it passes the FIPS-197 test vector. each cell is one byte of the 16-byte state; watch it scramble through ten rounds of SubBytes, ShiftRows, MixColumns and AddRoundKey, then decrypt back to the original.
          </p>
        </div>
      )}
      ...
      {tab === "sbox" && (
        <div className="aes-panel">
          <div className="aes-sbox-lookup">
            <label className="cx-field aes-sbox-input">
              <span>input byte (hex)</span>
              <input
                value={sboxIn}
                onChange={(e) => setSboxIn(e.target.value.replace(/[^0-9a-fA-F]/g, "").slice(0, 2))}
                aria-label="s-box input byte"
              />
            </label>
            <span className="aes-sbox-arrow">→</span>
            <span className="aes-sbox-out">0x{AES_SBOX[sboxByte].toString(16).padStart(2, "0")}</span>
            <span className="cx-note">non-linear and fixed. without it, AES would be linear and breakable.</span>
          </div>
          <div className="aes-sbox-grid">
            {AES_SBOX.map((out, i) => (
              <span
                key={i}
                className={`aes-sbox-cell ${i === sboxByte ? "is-active" : ""}`}
                style={{ background: `hsl(${(out * 1.41) % 360}, 60%, ${22 + (out / 255) * 30}%)` }}
                title={`0x${i.toString(16).padStart(2, "0")} maps to 0x${out.toString(16).padStart(2, "0")}`}
                onMouseEnter={() => setSboxIn(i.toString(16).padStart(2, "0"))}
              />
            ))}
          </div>
          <p className="widget-caption">
            all 256 S-box entries, one cell per input byte, coloured by output. it is a fixed one-to-one permutation built from the inverse in GF(2^8) plus an affine map. hover any cell to load that lookup above.
          </p>
        </div>
      )}
      ...
      {tab === "modes" && (
        <div className="aes-panel">
          <p className="cx-scenario">the same 16-byte block, &quot;ATTACK AT DAWN!!&quot;, repeated four times.</p>
          <div className="aes-modes">
            <div className="aes-mode">
              <span className="aes-mode-name">plaintext</span>
              {modes.plain.map((b, i) => blockRow(b, `p${i}`))}
              <span className="aes-mode-note">four identical blocks going in.</span>
            </div>
            <div className="aes-mode">
              <span className="aes-mode-name bad">ECB</span>
              {modes.ecb.map((b, i) => blockRow(b, `e${i}`))}
              <span className="aes-mode-note bad">identical out. the pattern survives. never use ECB.</span>
            </div>
            <div className="aes-mode">
              <span className="aes-mode-name ok">CBC</span>
              {modes.cbc.map((b, i) => blockRow(b, `c${i}`))}
              <span className="aes-mode-note">chained with an IV. all different.</span>
            </div>
            <div className="aes-mode">
              <span className="aes-mode-name ok">CTR</span>
              {modes.ctr.map((b, i) => blockRow(b, `t${i}`))}
              <span className="aes-mode-note">keystream per counter. all different, parallelisable.</span>
            </div>
          </div>
          <p className="widget-caption">
            each square is one ciphertext byte, coloured by value, so equal bytes share a colour. ECB encrypts identical plaintext blocks to identical ciphertext blocks, which is exactly the penguin leak. CBC and CTR break the pattern.
          </p>
        </div>
      )}
    </div>
